...
const parseDate = function (strDate) {
  ...
  if (strDate.indexOf('/Date(') > -1)
    ...
    myDate = new Date(
      Date.parse(strDate.replace(/-/g, '/').replace('T', ' ').split('.')[0])
    )
...
const getYesterday = function () {
  ...
  res.startTime = dateFormat(
    new Date(new Date().setDate(new Date().getDate() - 1)),
    'yyyy-MM-dd 00:00:00'
  )
  res.endTime = dateFormat(
    new Date(new Date().setDate(new Date().getDate() - 1)),
    'yyyy-MM-dd 23:59:59'
  )
  return res
...
Date.prototype.DateAdd = function (strInterval, Number) {
  ...
  switch (strInterval) {
    case 's':
      return new Date(Date.parse(dtTmp) + 1000 * Number) // 秒
    case 'n':
      return new Date(Date.parse(dtTmp) + 60000 * Number) // 分
    case 'h':
      return new Date(Date.parse(dtTmp) + 3600000 * Number) // 小时
    case 'd':
      return new Date(Date.parse(dtTmp) + 86400000 * Number) // 天
    case 'w':
      return new Date(Date.parse(dtTmp) + 86400000 * 7 * Number) // 星期
    case 'q':
      return new Date(
        dtTmp.getFullYear(),
